...
import type { ArchitectureDiagnostic, ArchitectureManifest } from "../architecture/manifest.js";
import {
...
import { createGitArchitectureDiff, validateGitRef } from "./git-snapshot.js";
import {
  createAction,
  createApplication,
  createFeature,
...
function diagnosticLine(entry: ArchitectureDiagnostic): string {
...
async function runCheck(
  ...
  for (const entry of manifest.diagnostics) stderr.write(`${diagnosticLine(entry)}\n`);
  if (errors.length > 0) {
...
function runExplain(args: readonly string[], inspector: ApplicationInspector, stdout: CliStream, stderr: CliStream): number {
  ...
  else stdout.write(route === null ? formatFeatureExplanation(feature!) : formatRouteExplanation(route));
  return 0;
}
...
export { createGitArchitectureDiff, validateGitRef } from "./git-snapshot.js";
export { createAction, createApplication, createFeature, createModel, createQuery } from "./scaffold.js";
